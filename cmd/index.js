...
const setHelp = (args) => args.help('h').alias('h', 'help');

const setDemands = (args) => args.demandCommand(1).demandOption(['i', 'o']);

const setExample =
    ([cmd, desc]) =>
    (args) =>
        args.example(
            `$0 ${cmd} ${options[0][0][0]} ${options[0][1]}\
                          ${options[1][0][0]} ${options[1][1]}`,
            desc,
        );
...
const setOption =
    ([[option, alias], desc]) =>
    (args) =>
        args.alias(option, alias).nargs(option, 1).describe(option, desc);
